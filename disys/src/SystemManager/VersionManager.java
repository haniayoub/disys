...
public class VersionManager {
	...
	public String addVersion(byte[] jarFile,String className){
		...
		Common.Logger.TraceInformation("adding new version "+newVer+" new Executer ["+className+"]");
		String newVerFile=getJarFileName(newVer);
		String classNameFile=getClassNameFileName(newVer);
		Common.Logger.TraceInformation("Saving New Update Jar to :"+newVerFile + " and "+classNameFile);
		try {
			...
			try {
				...
				Common.Logger.TraceWarning("Failed to Create Class Name File:"+classNameFile,e);
			}
		} catch (FileNotFoundException e1) {
			Common.Logger.TraceWarning("Failed to Create jar File:"+newVerFile,e1);
		}
	...
	public Version GetVersion(int version){
		...
		try {
			...
			Common.Logger.TraceWarning("Failed to read update File:"+getJarFileName(version),null);
			return null;
		}
	}
	private void SetLastVersion(){
		...
		try {
			...
			Common.Logger.TraceWarning("Failed to read update File:"+getJarFileName(lastVerNum),null);
	...
	private String GetClassName(int version) {
		...
		try {
			...
			Common.Logger.TraceWarning(FileName+" File Not Found .", e);
			return null;
		}
	 Common.Logger.TraceInformation("Loading Class Name from : "+FileName);	
