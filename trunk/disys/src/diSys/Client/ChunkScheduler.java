...
public class ChunkScheduler<TASK extends Item,RESULT extends Item> extends AWorker<Chunk<TASK>,Chunk<TASK>>  {
	...
	public Chunk<TASK> doItem(Chunk<TASK> task) {
		...
		if(executerAddress==null){
			diSys.Common.Logger.TraceWarning("Failed to schdule a Chunk : No executers Found", null);
			try {
				Thread.sleep(1000);
		...
		if(RemoteExecuter==null){
			diSys.Common.Logger.TraceWarning("Failed to schdule a Chunk :Connection Failed", null);
			return task;
		}
		...
		try {
			...
			diSys.Common.Logger.TraceWarning("Couldn't add Chunk to Remote Receiver:"+itemRecieverInfo.GetRmiAddress(), null);
			return task;
		}
