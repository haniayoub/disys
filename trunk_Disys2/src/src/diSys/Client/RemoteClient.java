...
public class RemoteClient<TASK extends Item, RESULT extends Item> {
	...
	public void AddTask(TASK task, String IP, int irPort, int rcPort) {
		clientSystem.AddTask(task, IP, irPort, rcPort);
		taskNum++;
	}
	public void AddLast(TASK task, String IP, int irPort, int rcPort) {
		clientSystem.AddLast(task, IP, irPort, rcPort);
		taskNum++;
	}
	public void AddFirst(TASK task, String IP, int irPort, int rcPort) {
		clientSystem.AddFirst(task, IP, irPort, rcPort);
		taskNum++;
	}
	
	/**
	 * run the task and block wait for result 
	 * @param task
	 * @return
	 * @throws Exception
	 */
