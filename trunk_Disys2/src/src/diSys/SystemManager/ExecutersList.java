...
public class ExecutersList {
	...
	public LinkedList<ExecuterRemoteInfo> LoadExecutersList() {
		...
		try {
			while ((Line = br.readLine()) != null) {
				...
				String name = args[3];
				$.add(new ExecuterRemoteInfo(ip, irPort, icPort, name));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void addExecuterToFile(String address, int irPort, int rcPort, String name) {

		LinkedList<ExecuterRemoteInfo> $ = LoadExecutersList();
		ExecuterRemoteInfo Eri = new ExecuterRemoteInfo(address, irPort, rcPort, name);
		if ($.contains(Eri))
			return;
		$.add(Eri);
		WriteListToFile($);
