...
public class SystemPanel extends javax.swing.JPanel {
	...
	private void startButtonMouseClicked(MouseEvent evt) {
		String hostName = this.TextBox_HostName.getText();
		int port = Integer.parseInt(this.TextBox_port.getText());
		SystemManagerUI.sysmRi=new RMIRemoteInfo(hostName,port,SystemManager.GlobalID);
		...
		if(SystemManagerUI.sysManager==null){
			...
			SaveInfoFile(hostName, port, this.infoFile);
	...
	public void SaveInfoFile(String hostName, int port, String infFile){
		try {
			String lines=hostName+"\r\n"+port;
			FileManager.WriteFile(infFile,lines.getBytes());
